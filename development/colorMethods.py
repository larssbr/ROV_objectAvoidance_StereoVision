...
def track(image):
    ...
    if centroid_x != None and centroid_y != None:
        ...
        cv2.circle(image, ctr, 4, centerCircle_Color)
...
def blocks_clahe(img):
    width, height = img.shape[:2][::-1]
    roiSize = 8
    widthINT = int(width/roiSize)
    heightINT = int(height/roiSize)

    widthRatio = np.linspace(0,width, widthINT) # accending order
    print widthRatio
    heightRatio = np.linspace(0,height, heightINT) # accending order
    print heightRatio
    i = 0
    ...
    for j in widthRatio:
        ...
        j = int(j)
        print j
        #for i in heightRatio[:len(heightRatio)-1]:
        while i<height:
            #i=i+heightINT
            i = heightRatio[i]
            print "i"
            ...
            i = int(i)
            print i
            #piece = img[i*roiSize:height - i*roiSize + roiSize, j*roiSize:width - i*roiSize + roiSize]
            piece = img[i*roiSize:height, j*roiSize:width]
            #piece = img[cv2.cvRect(i*roiSize, j*roiSize, roiSize, roiSize)]
            ...
            piece = claheAdjustImages(piece)

            # putting the image back together
            #img[cv2.cvRect(i*roiSize, j*roiSize, roiSize, roiSize)] = piece
            #img[i*roiSize:height - i*roiSize + roiSize, j*roiSize:width - i*roiSize + roiSize] = piece
            img[i*roiSize:height, j*roiSize:width] = piece
